...
class restapi {
	...
	public function setup($user, $pass, $version="v4", $resultsperpage=100, $url="https://restapi.surveygizmo.com/")
	{
		$this->user = $user;
		$this->u_pass = $pass;
	...
	public function buildURL($object)
	{
		...
		$call = $this->url . $this->version ."/". $slug . $this->format . "?user:pass=" . $this->user . ":" . $this->u_pass . "&resultsperpage=" . $this->resultsperpage . $this->filter;
	...
	public function get($object, $ids=NULL)
	{
		...
		do {
			$page = $i+1;
			$url = $this->buildURL($object)."&page={$page}";
